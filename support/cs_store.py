...
class CSStore:
    ...
    def _migrate(self) -> None:
        ...
        # Add last_seen_msg_id column if missing (added for unseen message tracking)
        cols = {r[1] for r in self._conn.execute("PRAGMA table_info(sessions)").fetchall()}
        if "last_seen_msg_id" not in cols:
            self._conn.execute("ALTER TABLE sessions ADD COLUMN last_seen_msg_id INTEGER DEFAULT 0")
            self._conn.commit()
    ...
    def set_last_seen(self, session_id: str, msg_id: int) -> None:
    ...
    def get_unseen_messages(self, session_id: str, limit: int = 50) -> list[dict]:
    ...
    def get_assigned_agent(self, session_id: str) -> str | None:
        row = self._conn.execute(
