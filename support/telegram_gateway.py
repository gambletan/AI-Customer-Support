"""
Telegram Gateway — Customer Service IM (Full-Featured)

Bridges customer messages (WebChat / WuKongIM / Telegram) to a Telegram
supergroup where human agents respond via topic threads.

Features:
- SQLite persistence (sessions, messages, ratings, tickets)
- Reconnect recovery + logged-in user topic reuse
- User online/offline notifications
- Agent typing indicator + idle-timeout reminders
...
- Slash commands: /erp, /order, /ticket, /tpl, /close, /history,
  /report, /hotwords, /queue, /transfer
- Least-load agent assignment
...
Configuration (env vars or ~/.env):
    ...
    MINIMAX_API_KEY="sk-..."               # translation + lang detect

Usage:
    ai-cs                    # via script entry point
...
async def _dequeue_next(manager: ChannelManager) -> None:
    ...
    assigned = auto_assign_agent(next_sid)
    if not assigned:
...
async def forward_to_user(manager: ChannelManager, msg: UnifiedMessage) -> None:
    ...
    # Persist
    msg_id = store.add_message(session_id, "agent", agent_text)
    ...
    store.set_last_seen(session_id, msg_id)
...
async def send_history(manager: ChannelManager, session_id: str) -> None:
    ...
    if hasattr(user_ch, '_sessions'):
        ...
        if ws and not ws.closed:
            ...
            store.set_last_seen(session_id, last_id)
