...
| 渠道 | 类型 | 用户如何接入 |
|------|------|-------------|
| **Telegram** | Bot API | 扫码私聊 Bot，直接对话 |
| **WhatsApp** | Cloud API | 扫码用自己的 WhatsApp 聊天 |
| **LINE** | Messaging API | 扫码加好友，LINE 内对话 |
| **Discord** | Bot | 在 Discord 私聊 Bot |
| **Slack** | Socket Mode | Slack 内直接对话 |
| **WeChat (Enterprise)** | 企业微信 API | 企业微信内对话 |
| **Feishu (Lark)** | 开放平台 | 飞书内对话 |
| **DingTalk** | 开放平台 | 钉钉内对话 |
| **MS Teams** | Bot Framework | Teams 内对话 |
| **QQ** | Bot API | QQ 内对话 |
| **Matrix** | 协议 | Element 等客户端对话 |
| **Zalo** | OA API | Zalo 内对话 (越南) |
| **iMessage** | macOS | iMessage 直接对话 |
| **WebChat** | WebSocket | 嵌入式网页聊天，支持图片/视频/语音/文档 |
| **WuKongIM** | HTTP 兼容 | 对接现有悟空 IM 移动端 APP |

**统一二维码：** 一个二维码 (`GET /qr`) → 渠道选择页 (`GET /connect`) → 用户选择自己常用的 IM 开始对话。无需安装额外 APP。
...
向导会引导你完成 6 步配置：Telegram Bot → 端口 → LLM 后端 → IM 渠道 → 客服设置 → 功能开关，自动生成 `.env` 文件。
...
- 渠道选择页: `http://localhost:8081/connect`
- 统一二维码: `http://localhost:8081/qr`
...
**最简单的方式：** 客户扫一个二维码，选择自己常用的 IM 开始对话。

```
用户手机                         你的服务器                      客服 Telegram 群组
  │                                │                              │
  ├─ 扫描二维码 ────────────────►  /connect 渠道选择页            │
  │  选择: Telegram/WhatsApp/..   │                              │
  │                                │                              │
...
把二维码放在网站、物料、包裹卡片上。用户扫码后看到所有已启用的 IM 渠道，点击即可开始对话。
...
1. **统一二维码** — 一个二维码所有渠道，用户扫码选择自己常用的 IM
2. **零前端开发** — WebChat 自带完整 UI，iframe 嵌入一行代码
3. **12 渠道即插即用** — 设环境变量即自动启用，无需写代码
4. **交互式配置** — `ai-cs setup` 向导式 6 步配置
5. **Mock ERP** — 无需真实 ERP 也能演示完整流程
6. **Feature Toggles** — 13 个功能开关，按需启用
7. **自动 Fallback** — LLM 后端自动降级

### 未来可进一步简化的方向

| 方向 | 说明 |
|------|------|
| **一键 Docker 部署** | `docker compose up` 即可运行，包含 Nginx 反向代理 + HTTPS |
| **WebChat JS SDK** | `<script src="chat.js">` 浮窗挂件，无需 iframe |
| **管理后台前端** | Dashboard 可视化页面（替代 curl API） |
| **Webhook 通知** | 新会话/超时/关闭时推送到企业系统 |
| **OAuth 单点登录** | 客户通过 SSO 自动绑定身份 |
...
详见 [API 对接文档](docs/API.md#4-im-渠道接入配置)，支持 12 个渠道，按环境变量自动启用。
...
support/
  cli.py                 # CLI 入口 (ai-cs)
  channels.py            # 12 渠道声明式注册表
  config.py              # 配置 + 功能开关
  setup.py               # 交互式配置向导 (6 步)
  gateway.py             # 网关启动 + 动态渠道加载 + 统一二维码
  state.py               # 共享运行时状态
